module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        try {
            existesOrError(category.name, 'nome nao informado')
        } catch (msg) {
    ...
    const remove = async (req, res) => {
        try {
            existesOrError(req.params.id, 'codigo da categoria ao informado')

            const subcategory = await app.db('categories')
                .where({ parentId: req.params.id })

            notExistsOrError(subcategory, 'categoria possua subcategorias')
            ...
            existesOrError(rowsDeleted, 'catgoria nao foi encontrada')
